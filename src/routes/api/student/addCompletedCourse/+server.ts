...
import { getCourseByCode } from '$lib/server/gmuCourseCatalog';
...
export async function POST({ request, cookies }: RequestEvent) {
  ...
  const catalogCourse = getCourseByCode(course.courseId);
  if (!catalogCourse) {
  ...
  try {
    // Add course to academic history
    await students.updateOne(
      { _id: userId },
